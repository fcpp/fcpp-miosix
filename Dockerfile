...
RUN add-apt-repository ppa:ubuntu-toolchain-r/test -y &&\
    ...
    apt-get -qq clean all
RUN apt-get -qq update &&\
    apt-get -qq -y install libstdc++6-i386-cross perl sudo &&\
    apt-get -qq clean all
RUN curl https://miosix.org/toolchain/MiosixToolchainInstaller.run --output MiosixToolchainInstaller.run &&\
